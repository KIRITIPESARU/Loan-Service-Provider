...
const Input = ({
  label, type = 'text', value, onChange, placeholder = '', error = '', required = false, className = '', ...props
}) => {
  return (
    <div className={`w-full flex flex-col ${className}`}>
      ...
      />
      {error && (
        <p className="text-sm text-red-600 mt-1.5 font-medium animate-slideDown">⚠️ {error}</p>
      )}
    </div>
  );
};
