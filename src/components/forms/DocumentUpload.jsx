  ...
  return (
    <div className="space-y-4">
      <div onDragOver={handleDragOver} onDrop={handleDrop} onClick={() => fileInputRef.current.click()}
        ...
        {selectedFile && (<p className="text-xs text-blue-600 font-medium bg-blue-50 px-2.5 py-0.5 rounded-full mt-2">File ready to upload ({(selectedFile.size / 1024 / 1024).toFixed(2)} MB)</p>)}
        <input type="file" ref={fileInputRef} onChange={handleFileChange} accept={accept} className="hidden" />
      </div>

      {selectedFile && (
        <div className="flex gap-2">
          <Button onClick={handleUploadClick} loading={loading} className="flex-1">Submit File</Button>
          <button onClick={() => setSelectedFile(null)} className="px-4 py-2 border border-gray-200 rounded-lg hover:bg-gray-50 text-gray-500 text-sm font-medium">Clear</button>
        </div>
      )}
    </div>
  );
